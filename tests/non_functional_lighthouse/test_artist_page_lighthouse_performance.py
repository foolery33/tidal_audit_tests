...
EXPECTED_ARTIST_PATH = "/artist/30416609"

REPORT_PATH = PROJECT_ROOT / "artifacts" / "lighthouse" / "artist_page_performance.json"
...
def run_lighthouse(url: str, report_path: Path) -> dict:
    ...
    assert completed.returncode not in LIGHTHOUSE_FATAL_EXIT_CODES, (
        f"Lighthouse не смог проверить страницу исполнителя. "
        f"URL: {url}. "
...
def test_artist_page_passes_basic_lighthouse_performance_check():
    lhr = run_lighthouse(ARTIST_URL, REPORT_PATH)

    runtime_error = lhr.get("runtimeError")

    assert runtime_error is None, (
        f"Lighthouse завершил проверку страницы исполнителя с runtimeError. "
        f"URL: {ARTIST_URL}. "
    ...
    assert parsed_final_url.path.rstrip("/") == EXPECTED_ARTIST_PATH, (
        f"Lighthouse проверил неожиданный маршрут вместо страницы исполнителя. "
        f"URL: {ARTIST_URL}. "
        f"Финальный URL: {final_url}. "
        f"Отчет: {REPORT_PATH}"
    )
    ...
    assert performance_score >= MIN_PERFORMANCE_SCORE, (
        f"Страница исполнителя имеет критически низкую Lighthouse performance-оценку. "
    ...
    assert not failed_critical_audits, (
        f"Страница исполнителя имеет критические замечания Lighthouse "
