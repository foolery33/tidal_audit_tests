...
REPORT_PATH = PROJECT_ROOT / "artifacts" / "lighthouse" / "home_page_performance.json"
...
def run_lighthouse(url: str, report_path: Path) -> dict:
    ...
    assert completed.returncode not in LIGHTHOUSE_FATAL_EXIT_CODES, (
        f"Lighthouse не смог проверить главную страницу. "
        f"URL: {url}. "
...
def test_home_page_passes_basic_lighthouse_performance_check():
    lhr = run_lighthouse(HOME_URL, REPORT_PATH)

    runtime_error = lhr.get("runtimeError")

    assert runtime_error is None, (
        f"Lighthouse завершил проверку главной страницы с runtimeError. "
        f"URL: {HOME_URL}. "
    ...
    assert urlparse(final_url).path.rstrip("/") in ("", "/"), (
        f"Lighthouse проверил неожиданный маршрут вместо главной страницы. "
        f"URL: {HOME_URL}. "
        f"Финальный URL: {final_url}. "
        f"Отчет: {REPORT_PATH}"
    )
    ...
    assert performance_score >= MIN_PERFORMANCE_SCORE, (
        f"Главная страница имеет критически низкую Lighthouse performance-оценку. "
    ...
    assert not failed_critical_audits, (
        f"Главная страница имеет критические замечания Lighthouse "
