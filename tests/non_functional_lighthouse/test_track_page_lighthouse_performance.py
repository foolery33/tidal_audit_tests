...
TRACK_URL = "https://tidal.com/album/369342737/track/369342742"
EXPECTED_TRACK_PATH = "/album/369342737/track/369342742"

REPORT_PATH = PROJECT_ROOT / "artifacts" / "lighthouse" / "track_page_performance.json"
...
def run_lighthouse(url: str, report_path: Path) -> dict:
    ...
    assert completed.returncode not in LIGHTHOUSE_FATAL_EXIT_CODES, (
        f"Lighthouse не смог проверить страницу трека. "
        f"URL: {url}. "
...
def test_track_page_passes_basic_lighthouse_performance_check():
    lhr = run_lighthouse(TRACK_URL, REPORT_PATH)

    runtime_error = lhr.get("runtimeError")

    assert runtime_error is None, (
        f"Lighthouse завершил проверку страницы трека с runtimeError. "
        f"URL: {TRACK_URL}. "
    ...
    assert parsed_final_url.path.rstrip("/") == EXPECTED_TRACK_PATH, (
        f"Lighthouse проверил неожиданный маршрут вместо страницы трека. "
        f"URL: {TRACK_URL}. "
        f"Финальный URL: {final_url}. "
        f"Отчет: {REPORT_PATH}"
    )
    ...
    assert performance_score >= MIN_PERFORMANCE_SCORE, (
        f"Страница трека имеет критически низкую Lighthouse performance-оценку. "
    ...
    assert not failed_critical_audits, (
        f"Страница трека имеет критические замечания Lighthouse "
