...
REPORT_PATH = PROJECT_ROOT / "artifacts" / "lighthouse" / "search_results_performance.json"
...
def run_lighthouse(url: str, report_path: Path) -> dict:
    ...
    assert completed.returncode not in LIGHTHOUSE_FATAL_EXIT_CODES, (
        f"Lighthouse не смог проверить страницу результатов поиска. "
        f"URL: {url}. "
...
def test_search_results_page_passes_basic_lighthouse_performance_check():
    lhr = run_lighthouse(SEARCH_URL, REPORT_PATH)

    runtime_error = lhr.get("runtimeError")

    assert runtime_error is None, (
        f"Lighthouse завершил проверку страницы результатов поиска с runtimeError. "
        f"URL: {SEARCH_URL}. "
    ...
    assert parsed_final_url.path.rstrip("/") == "/search", (
        f"Lighthouse проверил неожиданный маршрут вместо страницы результатов поиска. "
        f"URL: {SEARCH_URL}. "
        f"Финальный URL: {final_url}. "
        f"Отчет: {REPORT_PATH}"
    )

    assert final_query_params.get("q") == [SEARCH_QUERY], (
        f"Lighthouse проверил страницу поиска с неожиданным поисковым запросом. "
    ...
    assert performance_score >= MIN_PERFORMANCE_SCORE, (
        f"Страница результатов поиска имеет критически низкую Lighthouse performance-оценку. "
    ...
    assert not failed_critical_audits, (
        f"Страница результатов поиска имеет критические замечания Lighthouse "
